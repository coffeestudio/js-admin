...
var sidenav = (m: angular.IModule) => { m
    .directive('coffeeSideNav', () => {
        return {
            ...
            , scope: true
            , transclude: true
            , controller: ['$scope', '$coffee', '$state', ($scope, $coffee, $state) => {
            ...
            , link: (scope: any, elem, attr, ctrl, transclude) => {
                transclude(scope, template => {
                    elem.append(template);
                });
            }
        };
    })
    /* Stolen from jsfiddle */
    .directive('coffeeSideBlockTree', ($compile) => {
        return {
            restrict: 'A',
            transclude: 'element',
            priority: 100,
            terminal: true,
            compile: function (tElement, tAttrs: any, transclude) {
