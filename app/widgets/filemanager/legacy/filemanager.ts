...
/// <amd-dependency path="./upload-widget"/>
...
$['filemanager'] = function (e, o) {
    ...
    this.prevbox.click(function () { $(this).toggle(0) });
...
/*    var btn_up = $('<a/>');
    ...
    li = $('<li/>');
    var btn_upload: any = $('<a class="fa fa-upload"> загрузить</a>');
    btn_upload.addClass('btn_upload');
    this.panel.append(li.append(btn_upload));
    btn_upload.uploadWidget(
            { action: this.getAjaxUrl('upload')
            , callback: function (data) { console.log('Uploaded', data.value); that.cd(); }
            , init: function (form, opts) { console.log('Upload started'); opts.url = that.getAjaxUrl('upload'); }
            }
    );
...
fm.fn.extend({
    ...
    , getAjaxUrl: function (method, fn, custom_wd) {
        if (typeof(custom_wd) == 'undefined') custom_wd = this.cwd;
    ...
    , cd: function (dir) {
        ...
        if(typeof(dir) == 'undefined') dir = this.cwd;
        var that = this;
        this.cls();
        this.cwd = dir;
